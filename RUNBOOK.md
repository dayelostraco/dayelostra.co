...
- **www redirects happen at the edge, not in the bucket.** The CloudFront Function `dayelostra-co-www-redirect` (viewer-request on the default behavior) 301s `www.dayelostra.co` to `https://dayelostra.co` + path in one hop. The old path (Host-routed into the `www.dayelostra.co` redirect bucket) produced `301 http://dayelostra.co/index.html`: an http downgrade to a non-canonical URL. The bucket redirect still exists as a harmless fallback but should never be hit. Function source: `iac/cloudfront/www-redirect-function.js`. To update: `aws cloudfront update-function`, then `publish-function`, no distribution change needed.
