...
**Rate limits and discrete actions.** A human might call an internal API a dozen times an hour. An agent will call it ten thousand. So: no allow-alls, no catch-all permissions. Every action an agent takes should be a discrete, named operation with a discrete, named scope. "Read tickets in queue X" is a scope. "Read tickets," with no queue named, is a blank check. The same discipline holds one layer down at the OS: a policy that names which commands an agent may run forecloses destructive operations and closes the privilege-escalation paths, horizontal and vertical, in one move.
...
4. **What is the path to swapping the underlying model?** If the answer includes re-ATO, the architecture is fused to a vendor's release cadence, and that is a procurement problem misfiled as an engineering one.
